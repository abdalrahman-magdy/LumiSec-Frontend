...
export default function PortScanning() {
  ...
  const { result, loading, error, runScan, defaults } = usePortScanning();
  ...
  return (
    <>
      ...
      <PortConfiguration onScan={runScan} loading={loading} disabled={!canRunNetwork} defaults={defaults} />
      ...
      <div className="row g-3 justify-content-between align-items-stretch mb-4 mb-lg-5 px-2 mt-0">
        ...
        <div className="col dashboard-card">
          ...
          <ServiceDistribution
            results={openPorts}
            protocols={result?.services}
            showDemoFallback={!result}
          />
        </div>
      </div>
